...
4. Add a scheduled **Disaster recovery exercise** workflow.
   ...
   - It creates a migrated and seeded PostgreSQL source database, captures a custom-format dump, restores it into a distinct database, compares deterministic public-table row counts, validates migration status, and reruns migrations idempotently against the restored database.
