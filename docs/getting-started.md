...
1. **Identity:** configure the reference OIDC verifier or another `AccessTokenVerifier`, provide the production browser/session adapter, and define issuer, audience, permission mapping, token or session renewal, key rotation, and outage behavior. See `docs/oidc-authentication.md`.
